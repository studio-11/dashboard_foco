/**
 * script.js - Dashboard Course Check - VERSION MISE A JOUR avec colonnes de gestion
...
$(document).ready(function() {
     ...
     console.log('Script JS charge avec colonnes de gestion');
    ...
    setTimeout(function() {
        try {
            ...
            table = $('#coursesTable').DataTable({
                ...
                "deferRender": true,
                "columnDefs": [
                    { "orderable": false, "targets": [11, 17, 18, 19] },
                    { "className": "text-center", "targets": [0, 4, 5, 6, 7, 8, 10, 12, 13, 15, 16, 18, 19] },
            ...
            console.log('DataTables initialise avec succes');
   ...
   table = $('#coursesTable').DataTable({
    ...
    "order": [[1, 'asc']],
    "autoWidth": false,
    "destroy": true,        // AJOUTEZ CETTE LIGNE
    "columnDefs": [
        { "orderable": false, "targets": [11, 18, 19, 20] },
        { "className": "text-center", "targets": [0, 4, 5, 6, 7, 8, 10, 12, 13, 15, 16, 19, 20] },
        { "visible": false, "targets": [14] }  // Cache la colonne Format
    ],
    "drawCallback": function() {
        applyRowColoring();
        updateActiveFiltersDisplay();
    ...
    console.log('DataTables initialise avec colonnes de gestion');
    ...
    function setupFilters() {
        ...
        $('.description-filter').on('change', function() {
            console.log('Filtre description change');
            filterDescription();
        });

        $('.concepteur-filter').on('change', function() {
            console.log('Filtre concepteur change');
            filterConcepteur();
        });

        $('.gestionnaire-filter').on('change', function() {
            console.log('Filtre gestionnaire change');
            filterGestionnaire();
        });
    }
    ...
    function setupResetFilters() {
        $('#resetFilters').click(function() {
            ...
            console.log('Tous les filtres reinitialises');
        });
    }
    ...
    function filterByDate(date) {
        if (date) {
            ...
            table.draw();
            console.log('Filtre date applique: cours >= ' + date);
        } else {
            ...
            table.draw();
            console.log('Filtre date supprime');
        }
    }
    ...
    function filterByStartDays(days) {
        if (days && days > 0) {
            ...
            console.log('Filtre dans X jours applique: ' + targetDateStr);
        } else {
            ...
            console.log('Filtre dans X jours supprime');
        }
    }
    ...
    function filterByCategory(category) {
        if (category) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.success && response.course_ids && response.course_ids.length > 0) {
                        ...
                        console.log('Filtre categorie applique: ' + response.count + ' cours trouves pour ' + category);
                    } else {
                        $.fn.dataTable.ext.search.push(function(settings, data, dataIndex) {
                            if (settings.nTable.id !== 'coursesTable') return true;
                            return false;
                        ...
                        console.log('Aucun cours trouve pour la categorie ' + category);
                    }
                    table.draw();
                },
                error: function(xhr, status, error) {
                    ...
                    console.log('Filtre simple applique pour:', category);
                }
            });
        } else {
            ...
            removeCategoryFilter();
            table.draw();
            console.log('Filtre categorie supprime');
        }
    }
    ...
    function filterYesNo() {
        $('.yesno-filter').each(function() {
            ...
            if (isChecked) {
                if (column == 6) {
                    activeFilters[filterKey] = 'Suivi Achevement : ' + value;
                } else if (column == 7) {
                    activeFilters[filterKey] = 'Achevement de cours : ' + value;
                } else {
    ...
    function filterTeams() {
        ...
        if (checkedValues.length > 0) {
            ...
            table.draw();
            console.log('Filtre Teams applique:', checkedValues);
        } else {
            $.fn.dataTable.ext.search = $.fn.dataTable.ext.search.filter(function(fn) {
                return fn.toString().indexOf('filterTeamsCustom') === -1;
            });
            table.draw();
            console.log('Filtre Teams supprime');
        }
    }
    ...
    function filterDescription() {
        ...
        if (checkedValues.length > 0) {
            let courseIds = {
                'element_lien_ok': [6778, 6769, 6893, 6898, 6755, 6719, 6833, 6772, 6724, 6962, 6774, 6771, 6959, 7005],
                'element_lien_absent': [6725, 6768, 6587, 6591, 6590, 6593, 6588, 6589, 6586, 6592, 6782, 6594, 5628, 6375, 6374],
                'aucun_element': []
            };
            ...
            $.fn.dataTable.ext.search.push(function filterDescriptionCustom(settings, data, dataIndex) {
                ...
                return checkedValues.some(function(value) {
                    if (value === 'element_lien_ok') {
                        return courseIds.element_lien_ok.includes(courseId);
                    } else if (value === 'element_lien_absent') {
                        return courseIds.element_lien_absent.includes(courseId);
                    } else if (value === 'aucun_element') {
                        return !courseIds.element_lien_ok.includes(courseId) && 
                               !courseIds.element_lien_absent.includes(courseId);
                    }
                    return false;
                });
            });
            table.draw();
            console.log('Filtre Description applique:', checkedValues);
        } else {
            $.fn.dataTable.ext.search = $.fn.dataTable.ext.search.filter(function(fn) {
                return fn.toString().indexOf('filterDescriptionCustom') === -1;
            });
            table.draw();
            console.log('Filtre Description supprime');
        }
    ...
    function filterConcepteur() {
        ...
        $('.concepteur-filter:checked').each(function() {
            ...
            if (value === 'found') {
                activeFilters['concepteur_found'] = 'Concepteur : Trouve';
            } else if (value === 'missing') {
                activeFilters['concepteur_missing'] = 'Concepteur : Non trouve';
            }
        });
        ...
        if (checkedValues.length > 0) {
            ...
            table.draw();
            console.log('Filtre Concepteur applique:', checkedValues);
        } else {
            $.fn.dataTable.ext.search = $.fn.dataTable.ext.search.filter(function(fn) {
                return fn.toString().indexOf('filterConcepteurCustom') === -1;
            });
            table.draw();
            console.log('Filtre Concepteur supprime');
        }
    }
    ...
    function filterGestionnaire() {
        ...
        $('.gestionnaire-filter:checked').each(function() {
            ...
            if (value === 'found') {
                activeFilters['gestionnaire_found'] = 'Gestionnaire : Trouve';
            } else if (value === 'missing') {
                activeFilters['gestionnaire_missing'] = 'Gestionnaire : Non trouve';
            }
        });
        ...
        if (checkedValues.length > 0) {
            ...
            table.draw();
            console.log('Filtre Gestionnaire applique:', checkedValues);
        } else {
            $.fn.dataTable.ext.search = $.fn.dataTable.ext.search.filter(function(fn) {
                return fn.toString().indexOf('filterGestionnaireCustom') === -1;
            });
            table.draw();
            console.log('Filtre Gestionnaire supprime');
        }
    }
